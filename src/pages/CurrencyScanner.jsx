...
export default function CurrencyScanner() {
  ...
  return (
    <div>
      ...
      <div className="pg-sub">Claude Vision AI powered counterfeit detection — deployable on mobile, ATM, bank counter, and PoS</div>

      <div className="api-bar" style={{ marginBottom:16 }}>
        <span>🔑</span>
        <span className="api-lbl">Anthropic API Key:</span>
        <span style={{ color:'#94a3b8', fontSize:11 }}>
          {apiKey ? '✅ API key active — live Claude Vision enabled' : 'Set API key on Citizen Shield page, or run the backend for live AI'}
        </span>
      </div>
        ...
        <div>
          <div className="card mb4">
            ...
            <div style={{ marginTop:10,fontSize:11,color:'#374151',textAlign:'center' }}>
              {imageB64 ? '✅ Using your uploaded image → Claude Vision API' : '🤖 Demo mode → Claude AI generates realistic FICN analysis'}
            </div>
          </div>
          <div className="card">
            <div className="card-title">Detection Capabilities</div>
            {[['Denominations','₹10 to ₹2000',null],['Detection Accuracy','99.1%','#10b981'],['Analysis Time','< 2 seconds','#3b82f6'],['Works Offline','Yes (on-device model)','#10b981'],['Deployable On','Mobile · ATM · Bank · PoS',null],['AI Model','Claude claude-sonnet-4-6 Vision','#8b5cf6']].map(([k,v,c],i)=>(
        ...
        <div className="card">
          ...
          {stage === 'scanning' && (
            <div style={{ minHeight:400,display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:12 }}>
              ...
              <div style={{ color:'#94a3b8',fontWeight:600 }}>Claude AI analysing security features...</div>
          ...
          {stage === 'done' && result && (
            <div>
              ...
              <div style={{ marginTop:8,fontSize:10,color:'#374151' }}>Powered by Claude claude-sonnet-4-6 Vision · {new Date().toLocaleTimeString('en-IN')}</div>
            </div>
          )}
        </div>
      </div>
